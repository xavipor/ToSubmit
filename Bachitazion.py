...
class Bachitazion(object):
    def __init__(self, sizeOfBatch=128,pathT="/home/jdominguezmartinez/pruebas/Microbleeds/cmb-3dcnn-code-v1.0/data/TheirSizes/AllPatchesWithMicrobleedsTrain/"):
...
#	shuffle(self.listTrain)
    ...
    def nextBatch_T (self):
        ...
        X=np.zeros((currentBatchSize,self.myShape[2],self.myShape[1],self.myShape[0],1))
...
#            pdb.set_trace()
            patch=aux2.reshape([16,16,10],order='F').copy()
            patch2 = patch.transpose([2,1,0])
#            patch= aux2.reshape((1,10,1,16,16))
#            patch2 = patch[0,:,0,:,:] #Asi seria un 10,16,16
#            patch2 = patch2.transpose([0,2,1])
#            patch = patch*255.0
#            if "WO" in element:
#                auxY = np.array([1,0])
#            else:
